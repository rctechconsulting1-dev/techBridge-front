...
export default function ShopCtaVariants({ variant, themePack, settings }: Props) {
  ...
  const buttonText = settings?.cta_button_text ?? "Shop Now";
  const buttonUrl = settings?.cta_button_url ?? "#catalog";
  const fallbackSettings = settings
    ? { ...settings, cta_button_url: buttonUrl, cta_button_text: buttonText }
    : settings;

  if (variant === "shop_now") {
    return <CTASection settings={fallbackSettings} variant="minimal_inline" />;
  }
  ...
  return (
    <section id="cta" className="scroll-mt-20 py-20" style={{ backgroundColor: theme.cardBackground }}>
      <div className={`mx-auto max-w-6xl px-6 py-10 sm:px-10 lg:px-12 ${theme.panelRadiusClass}`} style={{ background: `linear-gradient(135deg, ${primary} 0%, ${accent} 100%)` }}>
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1.4fr_auto] lg:items-center">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.24em] text-white/70">Checkout CTA</p>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <Link href="#catalog" className={`${theme.buttonRadiusClass} border border-white/60 px-6 py-3 text-sm font-semibold uppercase tracking-wide text-white`}>
              Browse Catalog
            </Link>
          </div>
        </div>
