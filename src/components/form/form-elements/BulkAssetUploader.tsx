...
export default forwardRef<BulkAssetUploaderRef, BulkAssetUploaderProps>(
  function BulkAssetUploader(
    ...
    const addGeolocationExif = async (
      file: File,
      latitude: number,
      longitude: number,
    ): Promise<File> => {
      ...
      return new Promise((resolve, reject) => {
        ...
        reader.onload = () => {
          try {
            ...
            fetch(newDataUrl)
              .then((res) => res.blob())
              .then((blob) => resolve(new File([blob], jpegFile.name, { type: "image/jpeg" })))
    ...
    const compressImage = async (file: File): Promise<File> => {
      ...
      while (compressed.size > compressionTarget && quality > 0.1) {
        ...
        const nextCompressed = (await imageCompressor(file, quality)) as File | null;
        if (!nextCompressed) {
          break;
        }
