      ...
      <section id="team" className="scroll-mt-20 py-20" style={{ backgroundColor: theme.softBackground }}>
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="mb-10 text-center">
            ...
            <h2 className="mt-3 text-4xl font-bold text-gray-900">Meet the people driving the work forward</h2>
          </div>
          <div className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3">
            {team.map((member) => (
              <div
                key={member.id}
                className={`border p-7 shadow-sm ${theme.panelRadiusClass}`}
                style={{ backgroundColor: theme.cardBackground, borderColor: `${primary}22` }}
              >
                <h3 className="text-2xl font-semibold text-gray-900">{member.name}</h3>
                {member.title ? <p className="mt-2 text-xs font-semibold uppercase tracking-[0.24em]" style={{ color: primary }}>{member.title}</p> : null}
                {member.bio ? <p className="mt-5 text-sm leading-relaxed text-gray-600">{member.bio}</p> : null}
              </div>
            ))}
          </div>
        </div>
